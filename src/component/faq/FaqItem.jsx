...
const toggleAccortion =()=>{
  ...
  return (
   ...
   <div className="p-3 lg:p-5 rounded-[12px] border border-solid border-[#D9DCE2] mb-5 cursor-pointer">
    ...
    onClick={toggleAccortion}>
        <h4 className="text-[16px] leading-7 lg:text-[22px] lg:leading-8 text-headingColor">
            {item.question}
        </h4>
        <div className={`${
            isOpen && "text-white bg-black border-none"
...
{isOpen ? <AiOutlineMinus/> : <AiOutlinePlus/> }
    ...
    {isOpen && (
        <div className="mt-4">
            <p className="text-[14px] leading-6 lg:text;text-[16px] lg:leading-7 font-[400] text-textColor">
                {item.content}
            </p>
        </div>
    )
